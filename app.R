...
server <- function(input, output) {
  schoolsInput <- reactive({
    ...
    filter <- ifelse(length(input$school_select) > 0,
           gsub(" ", "+", paste0("FULLNAME+IN+%28%27", paste(input$school_select, collapse = "%27,%27"),"%27)")),
           "1=1")
  ...
  output$schoolmap <- renderLeaflet({
    leaflet() %>%
      ...
      # Since this is now a dataframe I had to tell leaflet which columns are the latitude and longitude.
  ...
  AwardAmountInput <- reactive({
    ifelse(length(input$DistrictSelect) > 0,
           ...
           "1=1"))) # 1=1 is only necessary for ESRI things, you actually don't need this
    url <- paste("https://data.lacounty.gov/resource/gut7-6rmk.json?where=", filter) # You had school district again in here for some reason. PRINT the URL's when you build them
